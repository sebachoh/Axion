...
export default async function Page() {
  const initialContent = await getPageContent('/academia/programacion');

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      <header>
        <h1 className="page-title" style={{ fontSize: '2.5rem' }}>Programación</h1>
      ...
      <div className="glass-panel" style={{ padding: '1rem', background: 'var(--glass-bg-hover)', color: 'var(--color-text-muted)', fontSize: '0.9rem', textAlign: 'center', borderRadius: 'var(--radius-sm)' }}>
        ✨ Los Widgets Interactivos de Programación pueden insertarse aquí arriba.
      </div>
      ...
      <div>
        ...
        <NotionEditor initialContent={initialContent} route="/academia/programacion" />
      </div>
    </div>
  );
}
